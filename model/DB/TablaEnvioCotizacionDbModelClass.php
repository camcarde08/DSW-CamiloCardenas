 ...
 * @author hruge
 */
class TablaEnvioCotizacionDbModelClass {
    ...
    function insertEnvioCotizacion($idCotizacion, $destino, $medio, $observaciones){
        
        $SQL = "INSERT INTO sgm_envio_cotizacion (`id_cotizacion`,`destino`,`medio`,`observaciones`) VALUES (?,?,?,?);";
    ...
    function getEnvioCotizacionByIdCotizacion($idCotizacion){
        ...
        $SQL = "SELECT * FROM sgm_envio_cotizacion WHERE id_cotizacion = ?;";
        $query = $this->dbClass->getConexion()->prepare($SQL) ;
        $query->bindParam(1, $idCotizacion);
